...
interface SplashCursorProps {
  ...
  BACK_COLOR?: ColorRGB;
  TRANSPARENT?: boolean;
  SPLASH_SPEED_THRESHOLD?: number; // <-- Added new prop
}
...
export default function SplashCursor({
  ...
  TRANSPARENT = true,
  SPLASH_SPEED_THRESHOLD = 0.0067 // <-- Default speed threshold (adjust as needed)
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    let config = {
      ...
      BACK_COLOR,
      TRANSPARENT,
      SPLASH_SPEED_THRESHOLD: SPLASH_SPEED_THRESHOLD! // <-- Added to config
    };
    ...
    function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: ColorRGB) {
      ...
      // Only register as "moved" (and thus trigger a splat) if speed exceeds the threshold
      pointer.moved = speed >= config.SPLASH_SPEED_THRESHOLD;
      pointer.color = color;
    }
...
const c = HSVtoRGB(hue, 1.0, 1.0);      c.r *= 0.15;
      c.g *= 0.15;
      c.b *= 0.15;
      return c;
    }
    ...
    BACK_COLOR,
    TRANSPARENT,
    SPLASH_SPEED_THRESHOLD
  ]);

  return (
    <div className=" hidden md:block fixed top-0 left-0 z-0 pointer-events-none w-full h-full">
